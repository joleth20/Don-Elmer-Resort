...
$total_pages_query = "SELECT COUNT(*) FROM tblincome";
...
$query = "SELECT * FROM tblincome ORDER BY id_income ASC LIMIT $offset, $records_per_page";
$query_run = mysqli_query($con, $query);
...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-header py-3">
            ...
            <h6 class="m-0 font-weight-bold text-primary">Manage Income</h6>
    ...
    <button class="d-none d-sm-inline-block btn btn-sm btn-primary shadow-sm" onclick="generatePDF()"><i
        ...
        $query = "SELECT * FROM tblincome ORDER BY $column $sort LIMIT $offset, $records_per_page";
    ...
    <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
        <thead>
            <tr>
            <th><a href="?column=id_income&sort=<?php echo $sort?>">ID</a></th>
                        <th><a href="?column=name&sort=<?php echo $sort?>">Name</a></th>
                        <th><a href="?column=account&sort=<?php echo $sort?>">Account</a></th>
                       <th><a href="?column=amount&sort=<?php echo $sort?>">Amount</a></th>
                       <th><a href="?column=date_income&sort=<?php echo $sort?>">Income Date</a></th>
            <th>Action</th>
            </tr>
        </thead>
        <tbody>
    <!-- End Header Table -->
       ...
       <!-- Search function -->
        ...
        if (isset($_GET['search'])) { 
            ...
            $query = "SELECT * FROM tblincome WHERE CONCAT(id_income, name, account, amount, date_income) LIKE '%$filtervalues%' ";
            $query_run = mysqli_query($con, $query);
            ...
            if (mysqli_num_rows($query_run) > 0) {
                while ($items = mysqli_fetch_assoc($query_run)) {
                    ?>
                    <tr>
                        <td><center><?= $items['id_income']; ?></center></td>
                        <td><center><?= $items['name']; ?></center></td>
                        <td><center><?= $items['account']; ?></center></td>
                        <td><center><?= $items['amount']; ?></center></td>
                        <td><center><?= $items['date_income']; ?></center></td>
    
                        <td id="actions">
                        <form action="income.php" method="post">
                                        <input type="hidden" name="delete_id" value="<?php echo $items['id_income']; ?>">
        ...
        if (!isset($_GET['search']) && mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                <tr>
                    ...
                    <td id="actions">
                    <form action="income.php" method="post">
                                        <input type="hidden" name="delete_id" value="<?php echo $row['id_income']; ?>">
                                        <button type="submit" name="delete_income" class="btn btn-danger"> DELETE</button>
                                    </form>
